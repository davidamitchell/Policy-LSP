...
#
# Provides log_debug, log_info, log_warn, log_error functions that write to
# stderr with ISO-8601 timestamps and a [LEVEL] prefix.  The caller script's
# name is included so log lines from governance_loop.sh and test_headless_agent.sh
# are distinguishable in combined output.
#
# Usage:
#   source "$(dirname "${BASH_SOURCE[0]}")/lib/logging.sh"
#
# Environment:
#   LOG_LEVEL   Minimum log level to emit: debug, info, warn, error (default: debug)
